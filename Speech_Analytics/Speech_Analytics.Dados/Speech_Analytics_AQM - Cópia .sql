...
SELECT
	...
	INTO #TEMP_CTL
	FROM AQM.RSTAQM_Current.dbo.media me (nolock)
		INNER JOIN AQM.RSTAQM_Current.dbo.media_custominfo mcut ON (me.mediaid = mcut.mediaid)
		INNER JOIN AQM.RSTAQM_Current.dbo.custominfo inf ON (inf.custominfoid = mcut.custominfoid)
		INNER JOIN AQM.RSTAQM_Current.dbo.iqmuser iq on (me.userid = iq.userid)
		INNER JOIN AQM.RSTAQM_Current.dbo.application ap on (me.applicationid = ap.applicationid)
		INNER JOIN AQM.RSTAQM_Current.dbo.mediatype ty on (me.mediatypeid = ty.mediatypeid)
	...
	INTO #TMP_GERAL_CTL
	from AQM.RSTAQM_Current.dbo.media me (nolock)
		INNER JOIN AQM.RSTAQM_Current.dbo.iqmuser iq (nolock) on (me.userid = iq.userid)
		INNER JOIN AQM.RSTAQM_Current.dbo.application ap (nolock) on (me.applicationid = ap.applicationid)
		INNER JOIN AQM.RSTAQM_Current.dbo.mediatype ty (nolock) on (me.mediatypeid = ty.mediatypeid)
		INNER JOIN AQM.RSTAQM_Current.dbo.queue que (nolock) on (me.takenqueueid = que.queueid)
...
WHERE      (DATEADD(HOUR,-3,CD.CallInsertDt) between @dataIni and @dataFim and CD.FarOffHookDt IS NOT NULL 
AND MD.AgentDispId not in(
3,94,95,108,122,123,130,131,129)  AND S.Service_Id in(
'3',
'4',
'5',
'6',
